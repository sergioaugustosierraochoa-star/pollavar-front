      ...
      <section className="mx-auto flex min-h-[calc(100vh-4rem)] w-full max-w-md flex-col justify-center">
        <div className="mb-8 text-center">
          ...
          <div className="flex items-center justify-center gap-3">
            <h1 className="text-3xl font-bold tracking-normal text-[#10B981]">PollaVAR</h1>
        ...
        >
          ...
          <div className="space-y-5">
            {isRegister ? (
              <>
                <Field label="Nombre" name="name" autoComplete="name" error={fieldErrors.name} onChange={() => clearFieldError("name")} placeholder="Nombre completo" />
                <Field label="Usuario" name="username" autoComplete="username" error={fieldErrors.username} onChange={() => clearFieldError("username")} placeholder="usuario" />
                <Field label="Correo" name="email" type="email" autoComplete="email" error={fieldErrors.email} onChange={() => clearFieldError("email")} placeholder="correo@ejemplo.com" />
              </>
            ) : (
              <Field label="Usuario o correo" name="identifier" autoComplete="username" error={fieldErrors.identifier} onChange={() => clearFieldError("identifier")} placeholder="usuario o correo@ejemplo.com" />
            )}
...
function Field({
  ...
  return (
    <label className="grid gap-2 text-sm font-medium text-slate-700">
      ...
      <input
        className="h-12 rounded-xl border border-[#e2e8f0] px-3 text-base text-[#0f172a] outline-none transition placeholder:text-slate-400 focus:border-[#22D3EE] focus:ring-4 focus:ring-[#22D3EE]/10"
        name={name}
        type={type}
      ...
      />
      {error ? <span className="text-xs font-medium text-[#b45309]">{error}</span> : null}
    </label>
  );
}
